...
double inputtime, *outputtime;
{
   ...
   c1 = (tagbits) * (draincap(Wcompn, NCH, 1) + draincap(Wcompn, NCH, 2)) + draincap(Wcompp, PCH, 1) + gatecap(Wmuxdrv12n + Wmuxdrv12p, 20.0) +
        cols * Cwordmetal;
            ...
            for (Ntspd = 1; Ntspd <= MAXSPD; Ntspd = Ntspd * 2)
            {
               for (Ntwl = 1; Ntwl <= 1; Ntwl = Ntwl * 2)
               {
                  for (Ntbl = 1; Ntbl <= MAXN; Ntbl = Ntbl * 2)
                  {
                     ...
                     if (organizational_parameters_valid(rows, columns, Ndwl, Ndbl, Nspd, Ntwl, Ntbl, Ntspd))
                     {
