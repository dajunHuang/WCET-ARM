 ...
 *
 * $Id: dlite.c,v 1.1.1.1 2000/11/29 14:53:54 cu-cs Exp $
 *
 * $Log: dlite.c,v $
...
#define MOD_SIZES \
  (MOD_BYTE | MOD_HALF | MOD_WORD | MOD_QWORD | MOD_FLOAT | MOD_DOUBLE | MOD_CHAR | MOD_STRING)
              ...
              counter_t cycle)   /* cycle count */
{
  ...
  for (bp = dlite_bps; bp != NULL; bp = bp->next)
  {
    ...
    {
    case pt_addr:
      if ((bp->class & ACCESS_EXEC) && !range_cmp_range(&bp->range, next_PC))
      {
      ...
      if ((bp->class & ACCESS_READ) && ((access & ACCESS_READ) && !range_cmp_range(&bp->range, addr)))
      {
      ...
      if ((bp->class & ACCESS_WRITE) && ((access & ACCESS_WRITE) && !range_cmp_range(&bp->range, addr)))
      {
