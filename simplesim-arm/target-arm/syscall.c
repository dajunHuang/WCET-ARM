				 ...
				 int traceable)		   /* traceable system call? */
{
	...
	case ARM_SYS_read:
	{
		...
		} while (/*nread*/ error_return == -1 && errno == EAGAIN);
...
case ARM_SYS_close:
{
	...
	if (/*fd*/ regs->regs_R[MD_REG_R0] == 0 || /*fd*/ regs->regs_R[MD_REG_R0] == 1 || /*fd*/ regs->regs_R[MD_REG_R0] == 2)
	{
...
case ARM_SYS_socket:
	...
	/* result */ regs->regs_R[MD_REG_R0] =
		socket(/* domain */ xlate_arg(__domain,
									  ...
									  "socket(family)"),
			   /* type */ xlate_arg(__type, socktype_map, N_ELT(socktype_map), "socket(type)"),
			   /* protocol */ xlate_arg(__protocol, family_map, N_ELT(family_map), "socket(proto)"));
...
case ARM_SYS_writev:
{
	...
	iov =
		(struct iovec *)malloc(/* iovcnt */ regs->regs_R[MD_REG_R2] * sizeof(struct iovec));
	if (!iov)
	...
	for (i = 0; i < /* iovcnt */ regs->regs_R[MD_REG_R2]; i++)
	{
		...
		mem_bcopy(mem_fn, mem, Read,
				  (/*iov*/ regs->regs_R[MD_REG_R1] + i * sizeof(struct osf_iovec)),
	...
	} while (/*result*/ error_return == -1 && errno == EAGAIN);
...
case OSF_SYS_setsockopt:
{
	...
	if (/* optval */ regs->regs_R[MD_REG_A3] != 0 && /* optlen */ regs->regs_R[MD_REG_A4] != 0)
	{
	...
	/* result */ regs->regs_R[MD_REG_V0] =
		setsockopt(/* sock */ (int)regs->regs_R[MD_REG_A0],
				   /* level */ xlate_arg((int)regs->regs_R[MD_REG_A1], socklevel_map, N_ELT(socklevel_map), "setsockopt(level)"),
				   /* optname */ xlate_arg((int)regs->regs_R[MD_REG_A2], map, nmap, "setsockopt(opt)"),
				   /* optval */ buf,
...
case ARM_SYS_poll:
{
	...
	for (i = 0; i < /* nfds */ regs->regs_R[MD_REG_R1]; i++)
	{
		/* copy target side pointer data into host side vector */
		mem_bcopy(mem_fn, mem, Read,
				  (/* fds */ regs->regs_R[MD_REG_R0] + i * sizeof(struct pollfd)),
	...
	for (i = 0; i < /* nfds */ regs->regs_R[MD_REG_R1]; i++)
	{
		/* copy target side pointer data into host side vector */
		mem_bcopy(mem_fn, mem, Write,
				  (/* fds */ regs->regs_R[MD_REG_A0] + i * sizeof(struct pollfd)),
				  &fds[i], sizeof(struct pollfd));
	}
...
case ARM_SYS_socketcall:
	...
	case ARM_SYS_LISTEN:
	{
		warn("invalid/unimplemented socket function, PC=0x%08p, winging it", regs->regs_PC);
		abort();
	}
	break;
	case ARM_SYS_ACCEPT:
	...
	case ARM_SYS_SETSOCKOPT:
	{
		...
		/* copy optval to host memory */
		if (/* optval */ __optval != 0 && /* optlen */ __optlen != 0)
		{
		...
		/* result */ regs->regs_R[MD_REG_R0] =
			setsockopt(/* sock */ __s,
					   /* level */ xlate_arg(__level, socklevel_map, N_ELT(socklevel_map), "setsockopt(level)"),
					   /* optname */ xlate_arg(__optname, map, nmap, "setsockopt(opt)"),
					   /* optval */ buf,
					   /* optlen */ __optlen);
