...
#define AGEN(C, URD, URN, SHAMT, SHFT, RM) \
  (0x00000000 | ((C) << 28) | ((URN) << 17) | ((URD) << 12) | ((SHAMT) << 7) | ((SHFT) << 5) | (RM))
...
#define ADD(C, RD, RN, SHAMT, SHFT, RM) \
  (0x00800000 | ((C) << 28) | ((RN) << 16) | ((RD) << 12) | ((SHAMT) << 7) | ((SHFT) << 5) | (RM))
#define ADDI(C, RD, RN, SHFT, IMM) \
  (0x02800000 | ((C) << 28) | ((RN) << 16) | ((RD) << 12) | ((SHFT) << 8) | (IMM))
#define SUB(C, RD, RN, SHAMT, SHFT, RM) \
  (0x00400000 | ((C) << 28) | ((RN) << 16) | ((RD) << 12) | ((SHAMT) << 7) | ((SHFT) << 5) | (RM))
#define SUBI(C, RD, RN, SHFT, IMM) \
  (0x02400000 | ((C) << 28) | ((RN) << 16) | ((RD) << 12) | ((SHFT) << 8) | (IMM))
#define STR(C, P, U, W, RD, RN, OFS) \
  (0x04000000 | ((C) << 28) | ((P) << 24) | ((U) << 23) | ((W) << 21) | ((RD) << 12) | ((RN) << 16) | (OFS))
#define LDR(C, P, U, W, RD, RN, OFS) \
  (0x04100000 | ((C) << 28) | ((P) << 24) | ((U) << 23) | ((W) << 21) | ((RD) << 12) | ((RN) << 16) | (OFS))
/* FIXME: bits missing! */
#define STFD(C, P, U, W, FD, RN, FPOFS) \
  (0x00000000 | ((C) << 28) | ((P) << 24) | ((U) << 23) | ((W) << 21) | ((FD) << 12) | ((RN) << 16) | (FPOFS))
...
int md_emit_ldst(struct md_uop_t *flow,
...
int md_get_flow(enum md_opcode op, md_inst_t inst,
...
int md_addc(word_t lhs, word_t rhs, word_t res)
{
  ...
  if (((lhs | rhs) >> 30) != 0)
    cflag = ((NEG(lhs) && NEG(rhs)) || (NEG(lhs) && POS(res)) || (NEG(rhs) && POS(res)));
  else
    cflag = 0;
...
int md_addv(word_t lhs, word_t rhs, word_t res)
{
  ...
  if (((lhs | rhs) >> 30) != 0)
    vflag = ((NEG(lhs) && NEG(rhs) && POS(res)) || (POS(lhs) && POS(rhs) && NEG(res)));
  else
    vflag = 0;
...
int md_subc(word_t lhs, word_t rhs, word_t res)
{
  ...
  if ((lhs >= rhs) || ((rhs | lhs) >> 31) != 0)
    cflag = ((NEG(lhs) && POS(rhs)) || (NEG(lhs) && POS(res)) || (POS(rhs) && POS(res)));
  else
    cflag = 0;
...
int md_subv(word_t lhs, word_t rhs, word_t res)
{
  ...
  if ((lhs >= rhs) || ((rhs | lhs) >> 31) != 0)
    vflag = ((NEG(lhs) && POS(rhs) && POS(res)) || (POS(lhs) && NEG(rhs) && NEG(res)));
  else
    vflag = 0;
...
int md_cond_ok(md_inst_t inst, word_t psr)
{
  int res;
  ...
  case COND_GT:
    res = ((!_PSR_N(psr) && !_PSR_V(psr) && !_PSR_Z(psr)) || (_PSR_N(psr) && _PSR_V(psr) && !_PSR_Z(psr)));
    break;

  case COND_LE:
    res = ((_PSR_N(psr) && !_PSR_V(psr)) || (!_PSR_N(psr) && _PSR_V(psr)) || _PSR_Z(psr));
    break;
...
/* disassemble an Alpha instruction */
void md_print_uop(enum md_opcode op,
                  md_inst_t inst, /* instruction to disassemble */
                  md_addr_t pc,   /* addr of inst, used for PC-rels */
                  FILE *stream)   /* output stream */
{
