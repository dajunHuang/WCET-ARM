...
/* open pipeline trace */
void ptrace_open(char *range,  /* trace range */
...
#define ptrace_check_active(PC, ICNT, CYCLE)                                           \
	((ptrace_outfd != NULL && !range_cmp_range1(&ptrace_range, (PC), (ICNT), (CYCLE))) \
...
#define ptrace_newstage(A, B, C, D, E) \
	if (ptrace_active)                 \
	__ptrace_newstage_lat((A), (B), (C), (D), (E))
...
void __ptrace_newstage_lat(unsigned int iseq,													/* instruction sequence number */
						   char *pstage,														/* pipeline stage entered */
...
void ptrace_print_stat(struct stat_sdb_t *sdb,	 /* stat database */
					   struct stat_stat_t *stat, /* stat variable */
					   FILE *fd);				 /* output stream */

/* print the value of all stat variables in stat database SDB */
void ptrace_print_stats(struct stat_sdb_t *sdb, /* stat database */
						FILE *fd);				/* output stream */
