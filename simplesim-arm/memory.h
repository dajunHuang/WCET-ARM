 ...
 *
 * $Id: memory.h,v 1.1.1.1 2000/11/29 14:53:54 cu-cs Exp $
 *
 * $Log: memory.h,v $
...
typedef enum md_fault_type (*mem_access_fn)(struct mem_t *mem, /* memory space to access */
...
#define MEM_PTE_ADDR(PTE, IDX) \
  (((PTE)->tag << (MD_LOG_PAGE_SIZE + MEM_LOG_PTAB_SIZE)) | ((IDX) << MD_LOG_PAGE_SIZE))
...
#define MEM_PAGE(MEM, ADDR)                                                                        \
  (/* first attempt to hit in first entry, otherwise call xlation fn */                            \
   ((MEM)->ptab[MEM_PTAB_SET(ADDR)] && (MEM)->ptab[MEM_PTAB_SET(ADDR)]->tag == MEM_PTAB_TAG(ADDR)) \
