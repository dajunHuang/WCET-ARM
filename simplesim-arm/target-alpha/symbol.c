  ...
  if ((bfd_get_file_flags(abfd) & (HAS_SYMS | HAS_LOCALS)))
  {
    ...
    for (i = 0; i < nsyms; i++)
    {
      ...
      if (/* from the data section */
          (!strcmp(sym->section->name, ".rdata") || !strcmp(sym->section->name, ".data") || !strcmp(sym->section->name, ".sdata") || !strcmp(sym->section->name, ".bss") || !strcmp(sym->section->name, ".sbss"))
    ...
    for (debug_cnt = 0, i = 0; i < nsyms; i++)
    {
          ...
          && RELEVANT_SCOPE(sym))
      {
        ...
        sym_db[debug_cnt].initialized =
            (!strcmp(sym->section->name, ".rdata") || !strcmp(sym->section->name, ".data") || !strcmp(sym->section->name, ".sdata"));
