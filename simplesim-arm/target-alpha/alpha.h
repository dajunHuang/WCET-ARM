 ...
 *
 * $Id: alpha.h,v 1.1.1.1 2000/11/29 14:53:54 cu-cs Exp $
 *
 * $Log: alpha.h,v $
...
#define MD_OUTPUT_SYSCALL(REGS) \
  ((REGS)->regs_R[MD_REG_V0] == OSF_SYS_write && ((REGS)->regs_R[MD_REG_A0] == /* stdout */ 1 || (REGS)->regs_R[MD_REG_A0] == /* stderr */ 2))
...
#define MD_EXO_TO_MISC_REGS(EXO, ICNT, REGS)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
  ...
  if (!exo || exo->ec != ec_list || !exo->as_list.head || exo->as_list.head->ec != ec_integer || !exo->as_list.head->next || exo->as_list.head->next->ec != ec_address || !exo->as_list.head->next->next || exo->as_list.head->next->next->ec != ec_address || !exo->as_list.head->next->next->next || exo->as_list.head->next->next->next->ec != ec_integer || !exo->as_list.head->next->next->next->next || exo->as_list.head->next->next->next->next->ec != ec_integer || exo->as_list.head->next->next->next->next->next != NULL) \
...
#define MD_VALID_ADDR(ADDR) \
  (((ADDR) >= ld_text_base && (ADDR) < (ld_text_base + ld_text_size)) || ((ADDR) >= ld_data_base && (ADDR) < ld_brk_point) || ((ADDR) >= (ld_stack_base - 16 * 1024 * 1024) && (ADDR) < ld_stack_base))
...
#define OVER(X, Y) (((((X) > 0) && ((Y) > 0) && (MAXINT_VAL - (X) < (Y)))  \
                         ? IFAIL("+ overflow")                             \
                         : (void)0),                                       \
...
#define UNDER(X, Y) (((((X) > 0) && ((Y) < 0) && (MAXINT_VAL + (Y) < (X)))  \
                          ? IFAIL("- overflow")                             \
                          : (void)0),                                       \
