				 ...
				 int traceable)		   /* traceable system call? */
{
	...
	case OSF_SYS_write:
	{
		...
		else
		{
			...
			} while (/*nwritten*/ regs->regs_R[MD_REG_V0] == -1 && errno == EAGAIN);
		}
	...
	case OSF_SYS_close:
		...
		if (/*fd*/ regs->regs_R[MD_REG_A0] == 0 || /*fd*/ regs->regs_R[MD_REG_A0] == 1 || /*fd*/ regs->regs_R[MD_REG_A0] == 2)
		{
			regs->regs_R[MD_REG_A3] = 0;
			break;
		}
...
#error No utimes() implementation!
	...
	case OSF_SYS_socket:
		...
		/* result */ regs->regs_R[MD_REG_V0] =
			socket(/* domain */ xlate_arg((int)regs->regs_R[MD_REG_A0],
										  ...
										  "socket(family)"),
				   /* type */ xlate_arg((int)regs->regs_R[MD_REG_A1], socktype_map, N_ELT(socktype_map), "socket(type)"),
				   /* protocol */ xlate_arg((int)regs->regs_R[MD_REG_A2], family_map, N_ELT(family_map), "socket(proto)"));
	...
	case OSF_SYS_writev:
	{
		...
		iov =
			(struct iovec *)malloc(/* iovcnt */ regs->regs_R[MD_REG_A2] * sizeof(struct iovec));
		if (!iov)
		...
		for (i = 0; i < /* iovcnt */ regs->regs_R[MD_REG_A2]; i++)
		{
			...
			mem_bcopy(mem_fn, mem, Read,
					  (/*iov*/ regs->regs_R[MD_REG_A1] + i * sizeof(struct osf_iovec)),
