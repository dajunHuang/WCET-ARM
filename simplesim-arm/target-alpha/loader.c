				  ...
				  int zero_bss_segs)	 /* zero uninit data segment? */
{
...

	{
		...
		for (sect = abfd->sections; sect; sect = sect->next)
		{
			...
			if ((bfd_get_section_flags(abfd, sect) & SEC_ALLOC) && (bfd_get_section_flags(abfd, sect) & SEC_LOAD) && bfd_section_vma(abfd, sect) && bfd_section_size(abfd, sect))
			{
			...
			if (!strcmp(bfd_section_name(abfd, sect), ".text"))
			{
				...
				ld_text_size =
					((bfd_section_vma(abfd, sect) + bfd_section_size(abfd, sect)) - MD_TEXT_BASE) + /* for speculative fetches/decodes */ TEXT_TAIL_PADDING;
	    ...
	    mem_bzero(mem_fn,
			...
			else if (!strcmp(bfd_section_name(abfd, sect), ".rdata") || !strcmp(bfd_section_name(abfd, sect), ".data") || !strcmp(bfd_section_name(abfd, sect), ".sdata") || !strcmp(bfd_section_name(abfd, sect), ".bss") || !strcmp(bfd_section_name(abfd, sect), ".sbss"))
			{
...

	{
		...
		else if (fhdr.f_magic == MD_SWAPH(ECOFF_EB_MAGIC) || fhdr.f_magic == MD_SWAPH(ECOFF_EL_MAGIC) || fhdr.f_magic == MD_SWAPH(ECOFF_EB_OTHER) || fhdr.f_magic == MD_SWAPH(ECOFF_EL_OTHER))
