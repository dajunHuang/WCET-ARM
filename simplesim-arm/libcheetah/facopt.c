					 ...
					 struct tree_node *ins_entry) /* deleted line */
{
	...
	else
	{
		...
		while (ptr)
		{
			...
			++top;
			p_stack[top] = ptr;
			if ((ptr->grpno < grpno) || ((ptr->grpno == grpno) && (ptr->prty > prty)))
				ptr = ptr->lft;
			else
			{
...
**********************************************************/
int process_groups(md_addr_t addr, /* address referenced */
...
Get_first_unknown(struct tree_node *entry, int search_prty, int del_prty)
{
	...
	while (top > 0)
	{
		...
		if ((ptr->prty > 0) || (ptr->grpno != grpno))
			return NULL;
		else if ((ptr->prty == search_prty) || ((ptr->prty < del_prty) && (ptr->prty > search_prty)))
			return ptr;
...
Get_first_unknown_wobacking(int grpno, int search_prty, int del_prty)
{
	...
	{
		++comp;
		if ((ptr->grpno < grpno) || ((ptr->grpno == grpno) && (ptr->prty > search_prty)))
		{
			lstlft_node = ptr;
			ptr = ptr->lft;
		}
