...
delete_node(void)
{
  ...
  if (first_time || top < 5 || delete_stack[top]->inum != last_inum || delete_stack[top - 1]->lft != delete_stack[top])
  {
    if (first_time)
    {
      first_time = 0;
 ...
 */
void ref_tree(unsigned i_inum, md_addr_t addr)
{
