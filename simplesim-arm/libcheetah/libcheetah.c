...
void cheetah_init(int argc, char **argv) /* cheetah arguments */
{
	short i;
	...
	for (i = 0; i < argc; i++)
	{
		...
		else
		{
			...
			case 'R':
				if ((NOT strcmp("lru", &argv[i][2])) || (NOT strcmp("LRU", &argv[i][2])))
					repl = LRU;
				else if ((NOT strcmp("opt", &argv[i][2])) || (NOT strcmp("OPT", &argv[i][2])))
				{
					repl = OPT;
				}
				else
			...
			case 'C':
				if ((NOT strcmp("fa", &argv[i][2])) || (NOT strcmp("FA", &argv[i][2])))
					trace_config = FA;
				else if ((NOT strcmp("sa", &argv[i][2])) || (NOT strcmp("SA", &argv[i][2])))
					trace_config = SA;
				else if ((NOT strcmp("dm", &argv[i][2])) || (NOT strcmp("DM", &argv[i][2])))
					trace_config = DM;
				else
				{
					fprintf(stderr,
...
void cheetah_config(FILE *fd) /* output stream */
{
...
void cheetah_access(md_addr_t addr) /* address of access */
{
	switch (trace_config)
	{
	case SA:
...
void cheetah_stats(FILE *fd, /* output stream */
