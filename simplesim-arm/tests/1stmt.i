...
rtx last_insn;
{
  ...
  if (cond_stack && (rtl_label == cond_stack->data.cond.else_label || rtl_label == cond_stack->data.cond.after_label))
    end_block = cond_stack;

  else if (loop_stack && (rtl_label == loop_stack->data.loop.start_label || rtl_label == loop_stack->data.loop.end_label || rtl_label == loop_stack->data.loop.continue_label))
...
static void
    fixup_gotos(thisblock, stack_level, cleanup_list, first_insn, dont_jump_in) struct nesting *thisblock;
rtx stack_level;
tree cleanup_list;
rtx first_insn;
int dont_jump_in;
{
  ...
  for (prev = 0, f = goto_fixup_chain; f; prev = f, f = f->next)
  {
    ...
    else if (((f->target_rtl)->fld[1].rtx) != 0)
    {
      ...
      if (f->target != 0 && (dont_jump_in || stack_level || cleanup_list) && ((first_insn)->fld[0].rtint) > ((f->before_jump)->fld[0].rtint) && !((f->target)->common.addressable_attr))
      {
      ...
      if (f->cleanup_list_list)
      {
        ...
        for (lists = f->cleanup_list_list; lists; lists = ((lists)->common.chain))
          ...
          if (((lists)->common.addressable_attr) && ((lists)->list.value) != 0)
...
int line;
{
  ...
  for (i = 0, tail = outputs; tail; tail = ((tail)->common.chain), i++)
  {
    ...
    if (((val)->common.code) != VAR_DECL && ((val)->common.code) != PARM_DECL && ((val)->common.code) != INDIRECT_REF)
    {
  ...
  for (tail = inputs; tail; tail = ((tail)->common.chain))
  {
    ...
    for (j = 0; j < ((((tail)->list.purpose))->string.length); j++)
      if (((((tail)->list.purpose))->string.pointer)[j] == '=' || ((((tail)->list.purpose))->string.pointer)[j] == '+')
      {
    ...
    ((body)->fld[3].rtvec->elem[i].rtx) = expand_expr(((tail)->list.value), 0, VOIDmode, 0);
    ((body)->fld[4].rtvec->elem[i].rtx) = gen_rtx(ASM_INPUT, ((((((tail)->list.value))->common.type))->type.mode),
  ...
  else
  {
    ...
    for (i = 0, tail = outputs; tail; tail = ((tail)->common.chain), i++)
    {
      ((body)->fld[0].rtvec->elem[i].rtx) = gen_rtx(SET, VOIDmode,
                                                    output_rtx[i],
    ...
    for (tail = clobbers; tail; tail = ((tail)->common.chain), i++)
    {
      ...
      ((body)->fld[0].rtvec->elem[i].rtx) = gen_rtx(CLOBBER, VOIDmode, gen_rtx(REG, QImode, j));
    }
        ...
        tree exp;
{
  ...
  if (extra_warnings && expr_stmts_for_value == 0 && !((exp)->common.volatile_attr) && exp != error_mark_node)
...
int exitflag;
{
  struct nesting *thiscond = (struct nesting *)xmalloc(sizeof(struct nesting));
...
void expand_end_cond()
{
  ...
  do
  {
    int initial_depth = nesting_stack->depth;
    do
    {
      struct nesting *this = cond_stack;
      cond_stack = this->next;
      nesting_stack = this->all;
      nesting_depth = this->depth;
      free(this);
    } while (nesting_depth > initial_depth);
  } while (0);
  last_expr_type = 0;
}
...
void
    expand_start_loop(exit_flag) int exit_flag;
{
  register struct nesting *thisloop = (struct nesting *)xmalloc(sizeof(struct nesting));
...
void
    expand_start_loop_continue_elsewhere(exit_flag) int exit_flag;
{
...
void expand_end_loop()
{
  ...
  if (optimize &&
      !(((insn)->code) == JUMP_INSN && ((((insn)->fld[3].rtx))->code) == SET && ((((insn)->fld[3].rtx))->fld[0].rtx) == pc_rtx && ((((((insn)->fld[3].rtx))->fld[1].rtx))->code) == IF_THEN_ELSE))
  {

    for (insn = loop_stack->data.loop.start_label; insn; insn = ((insn)->fld[2].rtx))
      if (((insn)->code) == JUMP_INSN && ((((insn)->fld[3].rtx))->code) == SET && ((((insn)->fld[3].rtx))->fld[0].rtx) == pc_rtx && ((((((insn)->fld[3].rtx))->fld[1].rtx))->code) == IF_THEN_ELSE &&
          ((((((((((insn)->fld[3].rtx))->fld[1].rtx))->fld[1].rtx))->code) == LABEL_REF && (((((((((insn)->fld[3].rtx))->fld[1].rtx))->fld[1].rtx))->fld[0].rtx) == loop_stack->data.loop.end_label)) ||
           (((((((((insn)->fld[3].rtx))->fld[1].rtx))->fld[2].rtx))->code) == LABEL_REF && (((((((((insn)->fld[3].rtx))->fld[1].rtx))->fld[2].rtx))->fld[0].rtx) == loop_stack->data.loop.end_label))))
        break;
    if (insn != 0)
    {
  ...
  do
  {
    ...
    do
    {
      struct nesting *this = loop_stack;
      loop_stack = this->next;
      nesting_stack = this->all;
      nesting_depth = this->depth;
      free(this);
    } while (nesting_depth > initial_depth);
...
int expand_exit_loop_if_false(cond)
tree cond;
{
  last_expr_type = 0;
  if (loop_stack == 0)
        ...
        tree retval;
{
  ...
  if (((retval)->common.code) == RESULT_DECL)
    retval_rhs = retval;
  else if ((((retval)->common.code) == MODIFY_EXPR || ((retval)->common.code) == INIT_EXPR) && ((((retval)->exp.operands[0]))->common.code) == RESULT_DECL)
  ...
  if (optimize && retval_rhs != 0 && frame_offset == 0 && ((retval_rhs)->common.code) == CALL_EXPR && ((((retval_rhs)->exp.operands[0]))->common.code) == ADDR_EXPR && ((((retval_rhs)->exp.operands[0]))->exp.operands[0]) == this_function
  ...
  if ((0) && !cleanups && !current_function_returns_pcc_struct)
  {
  ...
  if (cleanups && retval_rhs != 0 && ((retval_rhs)->common.type) != void_type_node && ((((((this_function)->decl.result))->decl.rtl))->code) == REG)
  {
    rtx last_insn;
...
void
    expand_start_bindings(exit_flag) int exit_flag;
{
  struct nesting *thisblock = (struct nesting *)xmalloc(sizeof(struct nesting));
  ...
  thisblock->data.block.outer_cleanups = (block_stack
        ...
        rtx rtl;
{
  ...
  else if (((rtl)->code) == MEM && ((((rtl)->fld[0].rtx))->code) == REG && ((rtl)->fld[0].rtx) != frame_pointer_rtx && ((rtl)->fld[0].rtx) != arg_pointer_rtx)
...
int dont_jump_in;
{
  ...
  if (dont_jump_in || thisblock->data.block.stack_level != 0 || thisblock->data.block.cleanups != 0)
  {
  ...
  do
  {
    ...
    do
    {
      struct nesting *this = block_stack;
      block_stack = this->next;
      nesting_stack = this->all;
      nesting_depth = this->depth;
      free(this);
    } while (nesting_depth > initial_depth);
...
void
    expand_decl(decl, cleanup) register tree decl;
tree cleanup;
{
  ...
  else if (((decl)->decl.size) == 0)
  {
    if (((decl)->decl.initial) == 0)
      ...
      ((decl)->decl.rtl) = gen_rtx(MEM, BLKmode, gen_reg_rtx(SImode));
           ...
           && !(flag_float_store && ((type)->common.code) == REAL_TYPE) && !((decl)->common.volatile_attr) && !((decl)->common.addressable_attr) && (((decl)->common.regdecl_attr) || !obey_regdecls))
  {
  ...
  else if (((((decl)->decl.size))->common.literal_attr))
  {
    ...
    if (((decl)->decl.rtl) != 0)
    {
      if (((((decl)->decl.rtl))->code) != MEM || ((((((decl)->decl.rtl))->fld[0].rtx))->code) != REG)
        abort();
      oldaddr = ((((decl)->decl.rtl))->fld[0].rtx);
    }

    ((decl)->decl.rtl) = assign_stack_local(((decl)->decl.mode),
                                            (((((decl)->decl.size))->int_cst.int_cst_low) * ((decl)->decl.size_unit) + 8 - 1) / 8);
    if (oldaddr)
    {
    ...
    ((((decl)->decl.rtl))->in_struct) = (((((decl)->common.type))->common.code) == ARRAY_TYPE || ((((decl)->common.type))->common.code) == RECORD_TYPE || ((((decl)->common.type))->common.code) == UNION_TYPE);
  }
  else
  {
    rtx address, size;
    ...
    {
      do_pending_stack_adjust();
      thisblock->data.block.stack_level = copy_to_reg(stack_pointer_rtx);
      stack_block_stack = thisblock;
    }
        ...
        tree decl;
{
  ...
  if (((decl)->decl.initial) == error_mark_node)
  {
    ...
    if (code == INTEGER_TYPE || code == REAL_TYPE || code == ENUMERAL_TYPE || code == POINTER_TYPE)
    ...
    cleanup, decl_elts;
{
  ...
  while (decl_elts)
  {
    ...
    ((decl_elt)->decl.rtl) = (((x)->mode) != BLKmode

                                  ? gen_rtx(SUBREG, ((((decl_elt)->common.type))->type.mode), x, 0)
                                  : x);

    if (cleanup != 0)
      thisblock->data.block.cleanups = temp_tree_cons(decl_elt, cleanup_elt,
                                                      thisblock->data.block.cleanups);
...
void move_cleanups_up()
{
  ...
  outer->data.block.cleanups = chainon(block->data.block.cleanups,
                                       outer->data.block.cleanups);
  block->data.block.cleanups = 0;
}

void
    expand_start_case(exit_flag, expr, type) int exit_flag;
tree expr;
tree type;
{
  register struct nesting *thiscase = (struct nesting *)xmalloc(sizeof(struct nesting));
...
void expand_end_case_dummy()
{
  do
  {
    ...
    do
    {
      struct nesting *this = case_stack;
      case_stack = this->next;
      nesting_stack = this->all;
      nesting_depth = this->depth;
      free(this);
    } while (nesting_depth > initial_depth);
...
int pushcase_range(value1, value2, label)
...
void check_for_full_enumeration_handling()
{
        ...
        tree orig_index;
{
  ...
  if (((index_expr)->common.type) != error_mark_node)
  {
    ...
    if (!thiscase->data.case_stmt.default_label && ((((orig_index)->common.type))->common.code) == ENUMERAL_TYPE && warn_switch)
      check_for_full_enumeration_handling();

    if (thiscase->data.case_stmt.default_label == 0)
    {
      thiscase->data.case_stmt.default_label = build_decl(LABEL_DECL, (tree)0, (tree)0);
    ...
    if (count == 0 || ((((index_expr)->common.type))->common.code) == ERROR_MARK)
    {
      ...
      emit_jump(default_label);
             ...
             || (unsigned)(((range)->int_cst.int_cst_low)) > 10 * count || ((index_expr)->common.code) == INTEGER_CST)
    {
      ...
      if (((index)->code) == CONST_INT || ((index_expr)->common.code) == INTEGER_CST)
      {
        ...
        for (n = thiscase->data.case_stmt.case_list;
             ...
             n = n->right)
        {
          if (!tree_int_cst_lt(index_expr, n->low) && !tree_int_cst_lt(n->high, index_expr))
            break;
        }
        if (n)
    ...
    else
    {
      ...
      for (n = thiscase->data.case_stmt.case_list; n; n = n->right)
      {
        register int i = ((n->low)->int_cst.int_cst_low) - ((minval)->int_cst.int_cst_low);

        while (i + ((minval)->int_cst.int_cst_low) <= ((n->high)->int_cst.int_cst_low))
          labelvec[i++] = gen_rtx(LABEL_REF, SImode, label_rtx(n->code_label));
      }
...
int unsignedp;
{
  if (((op1)->code) == CONST_INT && ((op2)->code) == CONST_INT)
  {
        ...
        case_node_ptr head;
{
  ...
  while (node)
  {
    ...
    while (((np = np->right) != 0) && next_real_insn(label_rtx(np->code_label)) == lb && tree_int_cst_equal(np->low, combine(PLUS_EXPR, node->high, build_int_2(1, 0))))
    {
      node->high = np->high;
    }
...
rtx
    assign_stack_local(mode, size) enum machine_mode mode;
int size;
{
  register rtx x, addr;
        ...
        tree decl;
{
  ...
  ((reg)->in_struct) = (((((decl)->common.type))->common.code) == ARRAY_TYPE || ((((decl)->common.type))->common.code) == RECORD_TYPE || ((((decl)->common.type))->common.code) == UNION_TYPE);
...
int toplevel;
{
  while (insn)
  {
    ...
    rtx note;
    if (((insn)->code) == INSN || ((insn)->code) == CALL_INSN || ((insn)->code) == JUMP_INSN)
    {

      if (toplevel && ((((insn)->fld[3].rtx))->code) == SET && ((((insn)->fld[3].rtx))->fld[0].rtx) == var && rtx_equal_p(((((insn)->fld[3].rtx))->fld[1].rtx), var))
      {
...
rtx insn;
{
  ...
  case SUBREG:
    tem = x;
    while (((tem)->code) == SUBREG || ((tem)->code) == SIGN_EXTRACT || ((tem)->code) == ZERO_EXTRACT)
      tem = ((tem)->fld[0].rtx);
    if (tem == var)
    {
      x = fixup_stack_1(x, insn);
  ...
  case SET:
    if (((((x)->fld[0].rtx))->code) == SIGN_EXTRACT || ((((x)->fld[0].rtx))->code) == ZERO_EXTRACT)
      optimize_bit_field(x, insn, 0);
    if (((((x)->fld[1].rtx))->code) == SIGN_EXTRACT || ((((x)->fld[1].rtx))->code) == ZERO_EXTRACT)
      optimize_bit_field(x, insn, 0);

    {
      ...
      while (((dest)->code) == SUBREG || ((dest)->code) == STRICT_LOW_PART || ((dest)->code) == SIGN_EXTRACT || ((dest)->code) == ZERO_EXTRACT)
        dest = ((dest)->fld[0].rtx);
      while (((src)->code) == SUBREG || ((src)->code) == SIGN_EXTRACT || ((src)->code) == ZERO_EXTRACT)
        src = ((src)->fld[0].rtx);

      if (src != var && dest != var)
        break;

      if ((((outerdest)->code) == SIGN_EXTRACT || ((outerdest)->code) == ZERO_EXTRACT) && ((((outerdest)->fld[0].rtx))->code) == SUBREG && ((((outerdest)->fld[0].rtx))->fld[0].rtx) == var)
        ((outerdest)->fld[0].rtx) = fixup_memory_subreg(((outerdest)->fld[0].rtx), insn);

      if ((((outersrc)->code) == SIGN_EXTRACT || ((outersrc)->code) == ZERO_EXTRACT) && ((((outersrc)->fld[0].rtx))->code) == SUBREG && ((((outersrc)->fld[0].rtx))->fld[0].rtx) == var)
        ((outersrc)->fld[0].rtx) = fixup_memory_subreg(((outersrc)->fld[0].rtx), insn);

      if ((((outerdest)->code) == SIGN_EXTRACT || ((outerdest)->code) == ZERO_EXTRACT) && ((((outerdest)->fld[0].rtx))->code) == MEM && ((((outerdest)->fld[0].rtx))->mode) != QImode)
      {
        ((outerdest)->fld[0].rtx) = copy_rtx(((outerdest)->fld[0].rtx));
        ((((outerdest)->fld[0].rtx))->mode = (QImode));
      }

      if ((((outersrc)->code) == SIGN_EXTRACT || ((outersrc)->code) == ZERO_EXTRACT) && ((((outersrc)->fld[0].rtx))->code) == MEM && ((((outersrc)->fld[0].rtx))->mode) != QImode)
      {
      ...
      if (((((x)->fld[1].rtx))->code) == REG || ((((x)->fld[0].rtx))->code) == REG || (((((x)->fld[1].rtx))->code) == SUBREG && ((((((x)->fld[1].rtx))->fld[0].rtx))->code) == REG) || (((((x)->fld[0].rtx))->code) == SUBREG && ((((((x)->fld[0].rtx))->fld[0].rtx))->code) == REG))
      {
  ...
  for (i = (rtx_length[(int)(code)]) - 1; i >= 0; i--)
  {
    ...
    if (fmt[i] == 'E')
    {
      ...
      for (j = 0; j < ((x)->fld[i].rtvec->num_elem); j++)
        ((x)->fld[i].rtvec->elem[j].rtx) = fixup_var_refs_1(var, ((x)->fld[i].rtvec->elem[j].rtx), insn);
    }
  }
  return x;
}
...
rtx insn;
{
  ...
  offset += ((((4) < ((mode_size[(int)(((((x)->fld[0].rtx))->mode))]))) ? (4) : ((mode_size[(int)(((((x)->fld[0].rtx))->mode))]))) - (((4) < ((mode_size[(int)(mode)]))) ? (4) : ((mode_size[(int)(mode)]))));
...
rtx insn;
{
  ...
  for (i = (rtx_length[(int)(code)]) - 1; i >= 0; i--)
  {
    ...
    if (fmt[i] == 'E')
    {
      register int j;
      for (j = 0; j < ((x)->fld[i].rtvec->num_elem); j++)
        ((x)->fld[i].rtvec->elem[j].rtx) = walk_fixup_memory_subreg(((x)->fld[i].rtvec->elem[j].rtx), insn);
    }
  }
  return x;
}
...
rtx insn;
{
  ...
  if (code == MEM)
  {
    ...
    if (((ad)->code) == PLUS && ((ad)->fld[0].rtx) == frame_pointer_rtx && ((((ad)->fld[1].rtx))->code) == CONST_INT)
    {
      rtx temp;
...
rtx *equiv_mem;
{
  ...
  if (((((body)->fld[0].rtx))->code) == SIGN_EXTRACT || ((((body)->fld[0].rtx))->code) == ZERO_EXTRACT)
    bitfield = ((body)->fld[0].rtx), destflag = 1;
  else
    bitfield = ((body)->fld[1].rtx), destflag = 0;

  if (((((bitfield)->fld[1].rtx))->code) == CONST_INT && ((((bitfield)->fld[2].rtx))->code) == CONST_INT && (((((bitfield)->fld[1].rtx))->fld[0].rtint) == (8 * mode_size[(int)(QImode)]) || ((((bitfield)->fld[1].rtx))->fld[0].rtint) == (8 * mode_size[(int)(HImode)])) && ((((bitfield)->fld[2].rtx))->fld[0].rtint) % ((((bitfield)->fld[1].rtx))->fld[0].rtint) == 0)
  {
    register rtx memref = 0;

    if (((((bitfield)->fld[0].rtx))->code) == MEM)
      memref = ((bitfield)->fld[0].rtx);
    else if (((((bitfield)->fld[0].rtx))->code) == REG && equiv_mem != 0)
      memref = equiv_mem[((((bitfield)->fld[0].rtx))->fld[0].rtint)];
    else if (((((bitfield)->fld[0].rtx))->code) == SUBREG && ((((((bitfield)->fld[0].rtx))->fld[0].rtx))->code) == MEM)
      memref = ((((bitfield)->fld[0].rtx))->fld[0].rtx);
    else if (((((bitfield)->fld[0].rtx))->code) == SUBREG && equiv_mem != 0 && ((((((bitfield)->fld[0].rtx))->fld[0].rtx))->code) == REG)
      memref = equiv_mem[((((((bitfield)->fld[0].rtx))->fld[0].rtx))->fld[0].rtint)];

    if (memref && !mode_dependent_address_p(((memref)->fld[0].rtx)) && offsetable_address_p(0, ((bitfield)->mode), ((memref)->fld[0].rtx)))
    {

      register int offset = ((((bitfield)->fld[2].rtx))->fld[0].rtint) / (8 * mode_size[(int)(QImode)]);
      if (((((bitfield)->fld[0].rtx))->code) == SUBREG)
      {
        offset += ((((bitfield)->fld[0].rtx))->fld[1].rtint) * 4;

        offset -= ((((4) < ((mode_size[(int)(((((bitfield)->fld[0].rtx))->mode))]))) ? (4) : ((mode_size[(int)(((((bitfield)->fld[0].rtx))->mode))]))) - (((4) < ((mode_size[(int)(((memref)->mode))]))) ? (4) : ((mode_size[(int)(((memref)->mode))]))));
      }
      ...
      if (destflag)
      {
        ((body)->fld[0].rtx) = adj_offsetable_operand(memref, offset);
        if (!(((((body)->fld[1].rtx))->code) == LABEL_REF || ((((body)->fld[1].rtx))->code) == SYMBOL_REF || ((((body)->fld[1].rtx))->code) == CONST_INT || ((((body)->fld[1].rtx))->code) == CONST))
        {
          ...
          while (((src)->code) == SUBREG && ((src)->fld[1].rtint) == 0)
            src = ((src)->fld[0].rtx);
          if (((src)->mode) != ((memref)->mode))
            src = gen_lowpart(((memref)->mode), ((body)->fld[1].rtx));
          ((body)->fld[1].rtx) = src;
        }
        else if (((((body)->fld[1].rtx))->mode) != VOIDmode && ((((body)->fld[1].rtx))->mode) != ((memref)->mode))
      ...
      else
      {
        ...
        while (((dest)->code) == SUBREG && ((dest)->fld[1].rtint) == 0)
          dest = ((dest)->fld[0].rtx);
        ((body)->fld[0].rtx) = dest;
        ...
        tree fndecl;
{
  ...
  int nparmregs = list_length(((fndecl)->decl.arguments)) + 56;

  int vararg = ((((fndecl)->decl.arguments) != 0 && ((((fndecl)->decl.arguments))->decl.name) && (!strcmp(((((((fndecl)->decl.arguments))->decl.name))->identifier.pointer), "__builtin_va_alist"))) ||
                (((fntype)->type.values) != 0 && (((tree_last(((fntype)->type.values)))->list.value) != void_type_node)));

  stack_args_size.constant = 0;
  stack_args_size.var = 0;

  if ((((((fndecl)->decl.result))->decl.mode) == BLKmode || 0) && ((struct_value_incoming_rtx)->code) == MEM)
  ...
  {
    int aggregate = (((((parm)->common.type))->common.code) == ARRAY_TYPE || ((((parm)->common.type))->common.code) == RECORD_TYPE || ((((parm)->common.type))->common.code) == UNION_TYPE);
    ...
    if (((parm)->common.type) == error_mark_node
        ...
        || ((parm)->common.code) != PARM_DECL || ((parm)->decl.arguments) == 0)
    {
    ...
    where_pad = (((passed_mode) == BLKmode ? (((
                                              ...
                                              ((
                                                   expand_expr(size_in_bytes(((parm)->decl.arguments)),
                                                               0, VOIDmode, 0))
                                                   ->fld[0]
                                                   .rtint) < ((target_flags & 040) ? 16 : 32) / 8)
                                           : (8 * mode_size[(int)(passed_mode)]) < ((target_flags & 040) ? 16 : 32))
                     ? downward
                     : upward);
    ...
    if (where_pad == downward)
    {
      if (passed_mode != BLKmode)
      {
        if ((8 * mode_size[(int)(passed_mode)]) % ((target_flags & 040) ? 16 : 32))
          stack_offset.constant += ((((8 * mode_size[(int)(passed_mode)]) + ((target_flags & 040) ? 16 : 32) - 1) / ((target_flags & 040) ? 16 : 32) * ((target_flags & 040) ? 16 : 32) / 8) - (mode_size[(int)(passed_mode)]));
      }
      else
      {
        ...
        {
          tree inc = (s2);
          if (((inc)->common.code) == INTEGER_CST)
            (stack_offset).constant += ((inc)->int_cst.int_cst_low);
          else if ((stack_offset).var == 0)
            (stack_offset).var = inc;
          else
            (stack_offset).var = genop(PLUS_EXPR, (stack_offset).var, inc);
        };
        {
          tree dec = (sizetree);
          if (((dec)->common.code) == INTEGER_CST)
            (stack_offset).constant -= ((dec)->int_cst.int_cst_low);
          else if ((stack_offset).var == 0)
            (stack_offset).var = genop(MINUS_EXPR, integer_zero_node, dec);
          else
            (stack_offset).var = genop(MINUS_EXPR, (stack_offset).var, dec);
        };
      }
    }
    ...
    entry_parm = 0;
    if (((((((parm)->common.type))->type.size))->common.code) == INTEGER_CST || stack_offset.var != 0)
    {

      entry_parm = (((target_flags & 020) && (args_so_far) < 8) ? gen_rtx(REG, (passed_mode), (args_so_far) / 4) : 0);
    }

    {
      ...
      nregs = (((target_flags & 020) && (args_so_far) < 8 && 8 < ((args_so_far) + ((passed_mode) == BLKmode ? int_size_in_bytes(((parm)->decl.arguments)) : (mode_size[(int)(passed_mode)])))) ? 2 - (args_so_far) / 4 : 0);
      ...
      if (((parm)->common.chain) == 0 && vararg && entry_parm != 0)
      {
        if (((entry_parm)->mode) == BLKmode)
          ...
          nregs = (int_size_in_bytes(((parm)->decl.arguments)) / 4);
      }

      if (nregs > 0)
      {
        current_function_pretend_args_size = (((nregs * 4) + (((target_flags & 040) ? 16 : 32) / 8) - 1) / (((target_flags & 040) ? 16 : 32) / 8) * (((target_flags & 040) ? 16 : 32) / 8));
    ...
    )
    {
      ...
      }
      {
        tree inc = (sizetree);
        if (((inc)->common.code) == INTEGER_CST)
          (stack_args_size).constant += ((inc)->int_cst.int_cst_low);
        else if ((stack_args_size).var == 0)
          (stack_args_size).var = inc;
        else
          (stack_args_size).var = genop(PLUS_EXPR, (stack_args_size).var, inc);
      };
    }
    else
      stack_parm = 0;

    if (nominal_mode != BLKmode && nominal_mode != passed_mode && stack_parm != 0)
    {

      if ((mode_size[(int)(nominal_mode)]) < 4)
      {
        stack_offset.constant += (mode_size[(int)(passed_mode)]) - (mode_size[(int)(nominal_mode)]);
        stack_offset_rtx = ((stack_offset).var == 0 ? gen_rtx(CONST_INT, VOIDmode, (stack_offset).constant) : plus_constant(expand_expr((stack_offset).var, 0, VOIDmode, 0), (stack_offset).constant));
      }
    ...
    if (nominal_mode == BLKmode)
    {
      if (((entry_parm)->code) == REG)
      {
        ...
        move_block_from_reg(((entry_parm)->fld[0].rtint), stack_parm,
                            int_size_in_bytes(((parm)->common.type)) / 4);
      }
      ((parm)->decl.rtl) = stack_parm;
    }
    else if (!((obey_regdecls && !((parm)->common.regdecl_attr) && !((fndecl)->common.inline_attr)) || ((parm)->common.addressable_attr) || ((parm)->common.volatile_attr)

               || (flag_float_store && ((((parm)->common.type))->common.code) == REAL_TYPE)))
    {
      ...
      if (nominal_mode == passed_mode && ((entry_parm)->code) == MEM && stack_offset.var == 0)
        ((get_last_insn())->fld[6].rtx) = gen_rtx(EXPR_LIST, REG_EQUIV,
...
rtx get_structure_value_addr(sizex)
rtx sizex;
{
  register int size;
        ...
        tree block;
{
  ...
  for (decl = ((block)->bind_stmt.vars); decl; decl = ((decl)->common.chain))
  {
    if (((decl)->common.code) == VAR_DECL
        ...
        && ((((decl)->common.type))->common.code) != RECORD_TYPE && ((((decl)->common.type))->common.code) != UNION_TYPE && ((((decl)->common.type))->common.code) != ARRAY_TYPE && ((((decl)->decl.rtl))->code) == REG && regno_uninitialized(((((decl)->decl.rtl))->fld[0].rtint)))
      warning_with_decl(decl,
                        "`%s' may be used uninitialized in this function");
    if (((decl)->common.code) == VAR_DECL && ((((decl)->decl.rtl))->code) == REG && regno_clobbered_at_setjmp(((((decl)->decl.rtl))->fld[0].rtint)))
      warning_with_decl(decl,
        ...
        tree block;
{
  register tree decl, sub;
  for (decl = ((block)->bind_stmt.vars); decl; decl = ((decl)->common.chain))
    if ((((decl)->common.code) == VAR_DECL || ((decl)->common.code) == PARM_DECL) && ((decl)->decl.rtl) != 0 && ((((decl)->decl.rtl))->code) == REG && !((decl)->common.regdecl_attr))
      put_var_into_stack(decl);
  for (sub = ((block)->stmt.body); sub; sub = ((sub)->common.chain))
    setjmp_protect(sub);
}
        ...
        tree subr;
{
  ...
  current_function_needs_context = (((current_function_decl)->decl.context) != 0 && ((((current_function_decl)->decl.context))->common.code) == LET_STMT);
  ...
  if (((((subr)->decl.result))->decl.mode) == BLKmode || 0 || (flag_pcc_struct_return && (((((((subr)->decl.result))->common.type))->common.code) == RECORD_TYPE || ((((((subr)->decl.result))->common.type))->common.code) == UNION_TYPE)))
  {
    ...
    ((((subr)->decl.result))->decl.rtl) = gen_rtx(MEM, ((((subr)->decl.result))->decl.mode),
                                                  value_address);
  }
  else

    ((((subr)->decl.result))->decl.rtl) = gen_rtx(REG, ((((((subr)->decl.result))->common.type))->type.mode), 0);
...
void
    expand_function_end(filename, line) char *filename;
int line;
{
  register int i;
  extern rtx sequence_stack;
  ...
  if (current_function_returns_struct)
  {
    ...
    rtx outgoing = hard_function_value(build_pointer_type(type),
