    ...
    for (stage = 0; stage < pipe_stages; stage++)
    {
        ...
        if ((stage == STAGE_EX))
        {
            ...
            if ((fu == IntALU) || (fu == RdPort) || (fu == WrPort))
            {
                ...
                node->num_fu = 1;
            node->lat.hi = fu_lat[fu].hi;
            node->lat.lo = fu_lat[fu].lo;
        }
...
inst_contends(int curr)
{
    ...
    for (inst = eg_chain[curr].pred; inst >= 0; inst = eg_chain[inst].pred)
    {
        ...
        else if (((src->fu == IntALU) || (src->fu == RdPort) || (src->fu == WrPort)) && (dst->fu == IntALU || (dst->fu == RdPort) || (dst->fu == WrPort)))
        {
...
void dump_coexist(int first, int last)
{
    int i;
