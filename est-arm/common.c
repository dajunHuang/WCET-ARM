...
void init_stack(Stack *stack, int elem_size)
{
...
void stack_push(Stack *stack, void *x)
{
...
void copy_stack(Stack *y, Stack *x)
{
    /* first free stack y */
    free_stack(y);
...
int range_isect(range_t *x, range_t *y)
{
    if (x->lo < y->lo)
        x->lo = y->lo;
    if (x->hi > y->hi)
...
// union operation
void range_union(range_t *x, range_t *y)
{
    if (x->lo > y->lo)
        x->lo = y->lo;
    if (x->hi < y->hi)
