...
int bbi_is_loopback(tcfg_edge_t *edge, int head, int tail)
{
    if ((edge->dst->bb->id == head) && (BETWEEN(edge->src->bb->id, head, tail)) && (edge->dst->bb->proc == edge->src->bb->proc))
        return 1;
    else
        return 0;
}
...
int bbi_is_return(tcfg_node_t *bbi)
{
